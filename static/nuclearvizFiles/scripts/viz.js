...
function set_up_map(view_model) {
    ...
    map.setView([40.4470, -79.9750], 13);
...
//ADD POINT
    ...
    chart_svg.append("g")
        .attr("class", "axis")
        .attr("transform","translate(-24, " + y_scale(0) + ")")
        .call(sc_x_axis);
...
$(document).ready(function() {
  ...
  if(datum.traffic == 0) {
    var trafficStr = ' --- ' + datum.traffic + ' trips/day';
  } else if (datum.traffic == 1) {
    if (datum.arrivals == 1) {
      var trafficStr = ' --- ' + datum.traffic + ' trips/day ' + 'which is an arrival';
    } else if(datum.departures == 1) {
      var trafficStr = ' --- ' + datum.traffic + ' trips/day ' + 'which is a departure';
    }
  } else {
    var trafficStr = ' --- ' + datum.traffic + ' trips/day of which ' +
  	    Math.round(datum.arrivals/datum.traffic*100) + ' % are arrivals';
  }


	var titleString = 'Total rides (sum of arrivals and departures) '
  ...
  $('#day').text(longhourMap[hour]);
    ...
    ko.computed(function() {
        ...
        if (arrivals && departures) {
            ...
            $('#capacity').text('[Rack Quantity: '+ capacity +']');
